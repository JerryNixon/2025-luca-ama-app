...
export default function InviteLinkPage({ params }: InviteLinkPageProps) {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      ...
      router.push(`/login?returnUrl=${encodeURIComponent(`/invite/${inviteLink}`)}`);
      return;
    }
  ...
  const joinEvent = async () => {
    try {
      ...
      const response = await apiClient.post(`/events/invite/${inviteLink}/`);
